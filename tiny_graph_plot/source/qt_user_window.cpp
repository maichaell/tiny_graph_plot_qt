...
QtUserWindow::QtUserWindow(GLFWwindow* window, const unsigned int w, const unsigned int h) :
    _window(window), _window_w(w), _window_h(h)
{
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
    //MIKA:glfwSetWindowUserPointer(_window, reinterpret_cast<void*>(this));
    this->SetCallbacks();
}

void QtUserWindow::SetCallbacks(void) const {
#ifdef DEBUG_WIN_ACTIONS
    printf("QtUserWindow::SetCallbacks\n");
#endif
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
    //MIKA:glfwSetFramebufferSizeCallback(_window, glfw_callback_functions::framebuffer_size_callback);
    //MIKA:glfwSetWindowRefreshCallback(_window, glfw_callback_functions::window_refresh_callback);
    //MIKA:glfwSetKeyCallback(_window, glfw_callback_functions::key_callback);
    //MIKA:glfwSetMouseButtonCallback(_window, glfw_callback_functions::mouse_button_callback);
    //MIKA:glfwSetCursorPosCallback(_window, glfw_callback_functions::mouse_pos_callback);
    //MIKA:glfwSetScrollCallback(_window, glfw_callback_functions::scroll_callback);
    //MIKA:glfwSetWindowPosCallback(_window, glfw_callback_functions::window_pos_callback);
    //MIKA:glfwSetWindowIconifyCallback(_window, glfw_callback_functions::window_iconify_callback);
    //MIKA:glfwSetWindowMaximizeCallback(_window, glfw_callback_functions::window_maximize_callback);
    //MIKA:glfwSetWindowFocusCallback(_window, glfw_callback_functions::window_focus_callback);
}

void QtUserWindow::framebuffer_size_event(int width, int height) {
#ifdef DEBUG_WIN_ACTIONS
    printf("QtUserWindow::framebuffer_size_event: %d, %d\n", width, height);
#endif
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
...
void QtUserWindow::window_pos_event(int xpos, int ypos) {
#ifdef DEBUG_WIN_ACTIONS
    printf("GLFW: window move: %d, %d\n", xpos, ypos);
#endif
}

void QtUserWindow::window_iconify_event(int iconified) {
#ifdef DEBUG_WIN_ACTIONS
    if (iconified) {
...
void QtUserWindow::window_maximize_event(int maximized) {
#ifdef DEBUG_WIN_ACTIONS
    if (maximized) {
...
void QtUserWindow::window_focus_event(int focused) {
#ifdef DEBUG_WIN_ACTIONS
    if (focused) {
...
void QtUserWindow::window_refresh_event() {
#ifdef DEBUG_WIN_ACTIONS
    printf("QtUserWindow::window_refresh_event\n");
#endif
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
    this->Clear();
    this->Draw();
    //MIKA:glfwSwapBuffers(_window);
}

void QtUserWindow::key_event(int key, int scancode, int action, int mods) {
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
#ifdef DEBUG_WIN_ACTIONS
    printf("QtUserWindow::key_event\n");
#endif
    /*if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
        //MIKA:glfwSetWindowShouldClose(_window, GLFW_TRUE);
    }
...
void QtUserWindow::mouse_button_event(int button, int action, int mods) {
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
...
void QtUserWindow::mouse_pos_event(double xs, double ys_inv) {
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
    const double ys = (double)_window_h - ys_inv;
...
void QtUserWindow::scroll_event(double xoffset, double yoffset) {
#ifdef SET_CONTEXT
    //MIKA:glfwMakeContextCurrent(_window);
#endif
#ifdef DEBUG_WIN_ACTIONS
    printf("QtUserWindow::scroll_event: % 0.6f\t% 0.6f\n", xoffset, yoffset);
#endif
