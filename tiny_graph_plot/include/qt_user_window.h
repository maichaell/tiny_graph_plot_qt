...
{
protected:
    QtUserWindow(GLFWwindow* window, const unsigned int w, const unsigned int h);
    QtUserWindow() {_window_h=0, _window_w=0;};
    virtual ~QtUserWindow(void) = 0 {};
private:
...
protected:
    ...
    virtual bool PointerInFrame(const double xs, const double ys)= 0;
    virtual void ClampToFrame(const double xs, const double ys,
                              double& o_xs, double& o_ys)= 0;
    virtual void SaveStartState(void) = 0;
    virtual void ToggleGraphVisibility(const int iGraph)= 0;
    virtual void Clear(void)= 0;
    virtual void Reshape(int p_width, int p_height) = 0;
    virtual void Draw(void) /*const*/ = 0;
    virtual void DrawCursor(const double xs, const double ys)= 0;
    virtual void DrawSelRectangle(const double xs0, const double ys0,
                                  const double xs1, const double ys1)= 0;
    virtual void DrawCircles(const double xs, const double ys)= 0;
