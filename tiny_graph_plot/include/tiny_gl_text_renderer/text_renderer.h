...
{
public:
    TextRenderer(QOpenGLContext* context);
    ~TextRenderer(void);
    void Init();
public:    
    ...
    void Draw(void) ;
    void DrawSingle(const size_t i_label) ;
    ...
    void SendToGPU(void) ;
    void SendToGPUverticesSingle(const size_t i_label) ;
    void SendToGPUtextureSingle(const size_t i_label) ;
private:
    unsigned int _w;
    unsigned int _h;
    GLuint _progID;
