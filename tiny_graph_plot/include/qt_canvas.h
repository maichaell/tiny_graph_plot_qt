...
using tiny_graph_plot::QtUserWindow;
using tiny_graph_plot::GraphF;
//using Histogram1dF;
using tiny_graph_plot::SizeInfo;

typedef unsigned int GLuint;

typedef tiny_graph_plot::Drawable<float> DrawableF;

template<typename T, typename VALUETYPE> class Histogram1d;
class SizeInfo;


class QtCanvas : public QOpenGLWidget, public QtUserWindow, protected QOpenGLExtraFunctions
{
...
public:
    //QtCanvas(QWidget* parent, const unsigned int w, const unsigned int h);
    explicit QtCanvas(QWidget* parent);
    virtual ~QtCanvas(void) override;
public:
    void AddGraph(const GraphF& p_graph);
    void AddHistogram(const tiny_graph_plot::Histogram1dF& p_histo);
...
private:
    ...
    virtual void Reshape(int p_width, int p_height) ;
    void PrintCursorValues(const double xs, const double ys);
    __forceinline void SwitchToFullWindow(void);
    __forceinline void SwitchToFrame(void);
    void AllocateBuffersForFixedSizedData(void);
    void SendFixedIndicesToGPU(void);
    int SendGridToGPU(void);
    void DrawGrid(void);
    void DrawAxes(void);
    void DrawVref(void);
    void SendFrameVerticesToGPU(void);
    void FillInFrame(void);
    void DrawFrame(void);
    void SendDrawableToGPU(const DrawableF* const p_graph, const SizeInfo& p_offset);
    void DrawDrawable     (const DrawableF* const p_graph, const SizeInfo& p_offset);
    virtual void DrawCursor(const double xs, const double ys);
    virtual void DrawSelRectangle(const double xs0, const double ys0,
                          const double xs1, const double ys1);
    virtual void DrawCircles(const double xs, const double ys);
    virtual void UpdateTexTextCur(const double xs, const double ys) ;
    virtual void UpdateTexTextRef(const double xs, const double ys) ;
...
private:
    ...
    // ----------------
private:
    std::vector<const GraphF*> _graphs;
    std::vector<const tiny_graph_plot::Histogram1dF*> _histograms;
    tiny_graph_plot::XYrange<float> _total_xy_range;
    tiny_graph_plot::XYrange<float> _visible_range;
    tiny_graph_plot::XYrange<float> _visible_range_start; //!< At mouse press
private:
    tiny_graph_plot::Grid<float> _grid;
private:
    ...
    virtual bool PointerInFrame(const double xs, const double ys);
    virtual void ClampToFrame(const double xs, const double ys,
                              double& o_xs, double& o_ys);
    virtual void SaveStartState(void) ;
    virtual void ToggleGraphVisibility(const int iGraph);
    void ExportPNG(const char* const dir, const char* const filename);
    ...
    __forceinline Vec4f TransformToVisrange(const double xs, const double ys,
        ...
        Vec4f* const o_in_viewport_space = nullptr);
    ...
    //    Vec4d* const o_in_clip_space = nullptr);
